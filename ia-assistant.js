...
class IAAssistant {
  ...
  prepareContext() {
    ...
    contexto += `detectar patrones alimenticios, recomendar recetas similares y siempre economicas con productos de facil acceso, `;
  ...
  addMessageToChat(role, message) {
    ...
    if (role === "user") {
      ...
      messageDiv.innerHTML = `
                ...
                <div class="message-content">${this.formatMarkdown(
                  message
                )}</div>
            `;
    }
  ...
  clearApiKey() {
    ...
    if (confirmDelete) {
      ...
      this.apiKey = "";
      alert(
        "✅ API Key borrada. Configura una nueva cuando quieras usar la IA."
      );
    }
  }
}
